...
def adicionar_tarefa():
    ...
    if tarefa and tarefa != "Escreva sua tarefa aqui":
        if frame_em_edicao is not None:
            atualizar_tarefa(tarefa) # type: ignore
            frame_em_edicao= None
        else:
...
def adicionar_item_tarefa(tarefa):
    ...
    botao_deletar= tk.Button(frame_tarefa, image=icon_deletar, command=lambda f=frame_tarefa: deletar_tarefa(f), bg="white",relief= tk.FLAT) # type: ignore
    botao_deletar.pack(side=tk.RIGHT, padx=5)

    frame_tarefa.pack(fill=tk.X, padx=5, pady=5)

    checkbutton= ttk.Checkbutton(frame_tarefa, command=lambda label=label_tarefa: alternar_sublinhado(label)) # type: ignore
...
def preparar_edicao(frame_tarefa, label_tarefa):
    ...
    for widget in frame_tarefa.winfo_children():
        if isinstance(widget, tk.Label):
            entrada_tarefa.insert(0, widget.cget("text"))
